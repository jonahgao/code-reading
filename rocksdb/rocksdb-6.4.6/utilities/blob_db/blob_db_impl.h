...
class BlobDBImpl : public BlobDB {
 ...
 private:
  ...
  bool SetSnapshotIfNeeded(ReadOptions* read_options);  // 填充ReadOptions的snapshot字段，如果snapshot字段之前没有设置
  ...
  std::map<uint64_t, std::shared_ptr<BlobFile>> blob_files_;  // 包含带ttl和不带ttl的所有blob文件

  // epoch or version of the open files.
  std::atomic<uint64_t> epoch_of_;    // 乐观锁
  ...
  std::set<std::shared_ptr<BlobFile>, BlobFileComparatorTTL> open_ttl_files_;  // 存储带 TTL KV 的可写入的blob文件，按照不同的过期时间范围组织，每个blob负责一个过期时间范围(BlobDBOption.ttl_range_secs)
  ...
  uint64_t fifo_eviction_seq_;  // 最近一次淘汰时当时的db seq

  // The expiration up to which latest FIFO eviction evicts.
  //
  // REQUIRES: access with metex_ lock held.
  uint64_t evict_expiration_up_to_;  // 最近一个淘汰时，淘汰到了哪个过期时间
