...
class BlobIndexCompactionFilter : public CompactionFilter {
 public:
  ...
  Decision FilterV2(int /*level*/, const Slice& key, ValueType value_type,
    ...
    if (blob_index.HasTTL() && blob_index.expiration() <= current_time_) {  // 过期的kv, 删除
      // Expired
      expired_count_++;
    ...
    if (!blob_index.IsInlined() &&
        ...
        context_.current_blob_files.count(blob_index.file_number()) == 0) {   // 找不到对应的blob文件(文件被gc或者被淘汰），删除
    ...
    if (context_.fifo_eviction_seq > 0 && blob_index.HasTTL() &&
        blob_index.expiration() < context_.evict_expiration_up_to) {   // 该blob已经被淘汰
      ...
      if (ok && ikey.sequence < context_.fifo_eviction_seq) {  // 小于fifo_eviction_seq，表示kv写入后执行的淘汰
        evicted_count_++;
        evicted_size_ += key.size() + value.size();
        return Decision::kRemove;
      }
