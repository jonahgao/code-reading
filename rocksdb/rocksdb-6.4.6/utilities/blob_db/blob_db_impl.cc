...
std::shared_ptr<BlobFile> BlobDBImpl::FindBlobFileLocked(
  ...
  auto citr = open_ttl_files_.equal_range(tmp);    // 查找过期范围包含expiration的blob文件
...
Status BlobDBImpl::SelectBlobFileTTL(uint64_t expiration,
  ...
  {
    ReadLock rl(&mutex_);
    *blob_file = FindBlobFileLocked(expiration);  // 查找过期范围包含expiration的blob文件
  ...
  uint64_t exp_low =
      (expiration / bdb_options_.ttl_range_secs) * bdb_options_.ttl_range_secs;  // 没找到，创建一个, 过期时间范围按照ttl_range_secs对
  ...
  *blob_file = NewBlobFile("SelectBlobFileTTL");
  ...
  if (epoch_of_.load() != epoch_read) {   // version变了，可能别人已经创建了包含expiration的blob文件,  再确认一次
...
Status BlobDBImpl::PutBlobValue(const WriteOptions& /*options*/,
  ...
  if (value.size() < bdb_options_.min_blob_size) {
    if (expiration == kNoExpiration) {
      ...
      // Inlined with TTL
      BlobIndex::EncodeInlinedTTL(&index_entry, expiration, value);  // key + expiration + value 编码到index_entry中.  之后作为一个BlobIndex类型的batch记录插入到WriteBatch中
    ...
    Slice value_compressed = GetCompressedSlice(value, &compression_output);  // 压缩，如果blob db选项里有配置

    std::string headerbuf;
    Writer::ConstructBlobHeader(&headerbuf, key, value_compressed, expiration);   // blob记录头部
    ...
    s = CheckSizeAndEvictBlobFiles(headerbuf.size() + key.size() +
                                   value_compressed.size());  // 检查是否超出max_db_size, 如果超出会清理过期或者比较旧的blob文件
    if (!s.ok()) {
      return s;
    }

    std::shared_ptr<BlobFile> blob_file;
    if (expiration != kNoExpiration) {   // 根据KV是否包含过期，选择不同的blob文件
    ...
    if (s.ok()) {
      ...
      s = AppendBlob(blob_file, headerbuf, key, value_compressed, expiration,
                     &index_entry);   // 写入blob文件，并返回blob记录的索引(index_entry)
    }
    if (s.ok()) {
      if (expiration != kNoExpiration) {
        blob_file->ExtendExpirationRange(expiration);
      }
      s = CloseBlobFileIfNeeded(blob_file);  // 是否blob文件写满了，需要关闭
    }
    if (s.ok()) {
      s = WriteBatchInternal::PutBlobIndex(batch, column_family_id, key,
                                           index_entry);   // 插入blob index到LSM树中的WriteBatch中
    }
    if (s.ok()) {
      if (expiration == kNoExpiration) {
...
Status BlobDBImpl::CheckSizeAndEvictBlobFiles(uint64_t blob_size,
  ...
  if (bdb_options_.max_db_size == 0 ||
      live_sst_size + total_blob_size_.load() + blob_size <=
          bdb_options_.max_db_size) {   // 是否超过最大db size的限制（如果有配置），包括所有sst file的总大小和blob file的总大小
    return Status::OK();
  }
