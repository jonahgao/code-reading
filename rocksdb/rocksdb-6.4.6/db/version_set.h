...
extern bool SomeFileOverlapsRange(const InternalKeyComparator& icmp,
                                  bool disjoint_sorted_files,             // disjoint_sorted_files: 各个文件范围不相交，且按范围排好序的
...
class VersionStorageInfo {
 public:
  ...
  bool RangeMightExistAfterSortedRun(const Slice& smallest_user_key,
                                     const Slice& largest_user_key,
                                     int last_level, int last_l0_idx);   //  检查范围是否会出现在比last_l0_idx老的L0文件或者比last_level更高的level
 ...
 private:
  ...
  std::vector<FileMetaData*>* files_;   // 各个层次的SST文件，按照key增序排列

  // Level that L0 data should be compacted to. All levels < base_level_ should
  // be empty. -1 if it is not level-compaction so it's not applicable.
  int base_level_;    // L0 compact到哪一层
  ...
  std::vector<std::vector<int>> files_by_compaction_pri_;    // 每层文件按file size从大到小排序，vector存放文件在files_中的index
  ...
  // ComputeCompactionScore()
  autovector<std::pair<int, FileMetaData*>> files_marked_for_compaction_;   // 每层待compact的文件
  ...
  autovector<std::pair<int, FileMetaData*>> bottommost_files_;   // 文件key范围跟所有下层文件范围（或者没有更下层的文件）不重叠
  autovector<std::pair<int, FileMetaData*>>
