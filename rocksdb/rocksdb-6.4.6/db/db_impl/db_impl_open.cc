...
DBOptions SanitizeOptions(const std::string& dbname, const DBOptions& src) {
  ...
  if (result.rate_limiter.get() != nullptr) {  // 如果启用限速，bytes_per_sync设为1M TODO:为啥
    if (result.bytes_per_sync == 0) {
      result.bytes_per_sync = 1024 * 1024;
    }
  }

  if (result.delayed_write_rate == 0) {     // delayed_write_rate默认值: 使用rate_limiter(如果有指定)；或者一个固定值
  ...
  if (result.wal_dir.back() == '/') {    // 去除目录末尾无用的斜杠
  ...
  if (result.use_direct_reads && result.compaction_readahead_size == 0) {   // 如果是使用direct_io（没有page cache加速），compaction时自己多读一些
    TEST_SYNC_POINT_CALLBACK("SanitizeOptions:direct_io", nullptr);
    result.compaction_readahead_size = 1024 * 1024 * 2;
  }

  if (result.compaction_readahead_size > 0 || result.use_direct_reads) {   // 如果compaction有特殊的read策略，则compaction读文件时为其建立单独的table_reader
    result.new_table_reader_for_compaction_inputs = true;                  // 不与执行用户查询的table reader复用，以免互相干扰
  }
