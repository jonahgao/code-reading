...
struct BatchContentClassifier : public WriteBatch::Handler {   // 迭代WriteBatch，用于计算存在哪些类型
...
WriteBatch::WriteBatch(size_t reserved_bytes, size_t max_bytes)
  ...
  rep_.reserve((reserved_bytes > WriteBatchInternal::kHeader)
                   ? reserved_bytes
                   : WriteBatchInternal::kHeader);   // 最少预分配一个header(8字节seq + 4字节count)长度
  rep_.resize(WriteBatchInternal::kHeader);  // header占位
}
...
void WriteBatch::Clear() {
  rep_.clear();
  rep_.resize(WriteBatchInternal::kHeader);  // 保持初始状态即有header的空间
...
uint32_t WriteBatch::ComputeContentFlags() const {
  ...
  if ((rv & ContentFlags::DEFERRED) != 0) {   // 需要延迟计算,遍历计算 TODO:
...
bool ReadKeyFromWriteBatchEntry(Slice* input, Slice* key, bool cf_record) {   // 从Batch的条目中读出key
  assert(input != nullptr && key != nullptr);
  // Skip tag byte
  input->remove_prefix(1);   // tag，即type 类型信息

  if (cf_record) {    // cf类型的记录，跳过cf id
...
Status ReadRecordFromWriteBatch(Slice* input, char* tag,
                                ...
                                Slice* value, Slice* blob, Slice* xid) {   // 从batch(input)中读取decode一条记录
...
Status WriteBatch::Iterate(Handler* handler) const {  // 遍历并使用handler处理Batch内的记录
  Slice input(rep_);
  if (input.size() < WriteBatchInternal::kHeader) {
    return Status::Corruption("malformed WriteBatch (too small)");
  }

  input.remove_prefix(WriteBatchInternal::kHeader);   // 跳过Batch的Header
...
Status WriteBatchInternal::CheckSlicePartsLength(const SliceParts& key,
                                                 const SliceParts& value) {  // 保证key，value数组的总长度不会越界uint32
...
class MemTableInserter : public WriteBatch::Handler {  // 迭代Writebatch内的记录插入到Memtable
