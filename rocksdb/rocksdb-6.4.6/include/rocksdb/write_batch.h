...
class WriteBatch : public WriteBatchBase {
 ...
 private:
  ...
  std::unique_ptr<SavePoints> save_points_;  // 栈，保存所有保存点
  ...
  // the WAL.
  SavePoint wal_term_point_; // 用于指定前面多少内容将写入WAL, TODO: 为啥

  // For HasXYZ.  Mutable to allow lazy computation of results
  mutable std::atomic<uint32_t> content_flags_; // 记录batch内是否存在某个类型的Record(如HasPut)
