...
class MemTableRep {
 public:
  ...
  class KeyComparator {  // 用于比较handle(里面有key，有value）的key部分 需要先读key长度，再解出key，再比较key
   public:
  ...
  virtual KeyHandle Allocate(const size_t len, char** buf);   // 由memtable的实现里分配内存，方便优化（比如使用连续内存）

  // Insert key into the collection. (The caller will pack key and value into a
  // single buffer and pass that in as the parameter to Insert).
  // REQUIRES: nothing that compares equal to key is currently in the
  // collection, and no concurrent modifications to the table in progress
  virtual void Insert(KeyHandle handle) = 0;    // handle里有打包好的key(internal)和value
  ...
  virtual void Get(const LookupKey& k, void* callback_args,
                   bool (*callback_func)(void* arg, const char* entry));   // 如果callback_func返回false，则结束；否则继续寻找下一个key
