...
class WriteBatchBase {
 public:
  ...
  // replication.
  virtual Status PutLogData(const Slice& blob) = 0;   // 会存储到事物日志。但不会持久化到其他文件（如SST）, 不消耗seq，不增加batch条目个数 用例：添加用于复制的时间戳
  ...
  virtual void SetSavePoint() = 0;   // 为当前状态设置一个保存点,便于后续回滚

  // Remove all entries in this batch (Put, Merge, Delete, PutLogData) since the
  // most recent call to SetSavePoint() and removes the most recent save point.
  // If there is no previous call to SetSavePoint(), behaves the same as
  // Clear().
  virtual Status RollbackToSavePoint() = 0;  // 回滚到上次设置的保存点

  // Pop the most recent save point.
  // If there is no previous call to SetSavePoint(), Status::NotFound()
  // will be returned.
  // Otherwise returns Status::OK().
  virtual Status PopSavePoint() = 0;  // 删除最近的保存点。没有会出错NotFound
